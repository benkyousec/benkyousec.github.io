...
martin is configured to execute `/usr/bin/backy.sh` with sudo privileges. This is a custom script and I would typically expect this to be placed under `/usr/local/bin`  instead of `/usr/bin` 🧐.
