...
- To prevent SSTI vulnerabilities, it is suggested to sanitise untrusted inputs for dangerous characters such as {% raw %} ${{<%[%'"}}%\\. {% endraw %} before they are rendered in templates. If sanitising the dangerous characters is not feasible due to business requirements, it is recommended to use a sandbox environment for hosting the application.
